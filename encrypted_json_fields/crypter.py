...
def build_crypter(keys):
    """
    Given a list of keys (or a key) build the corresponding crypter
    """
...
def encrypt_values(data, keys=None, json_skip_keys=None):
    # Inspired by:
    ...
    if isinstance(data, (list, tuple, set)):
        return [encrypt_values(x, keys, json_skip_keys) for x in data]

    # Scan the dicts, then encode each item recursively
    if isinstance(data, dict):
        return {
            key: encrypt_values(value, keys, json_skip_keys)
    ...
    encrypted_data = encrypt_str(repr(data), keys)
...
def decrypt_values(data, keys=None):
    ...
    if isinstance(data, (list, tuple, set)):
        return [decrypt_values(x, keys) for x in data]

    # Scan the dicts, then decode each item recursively
    if isinstance(data, dict):
        return {key: decrypt_values(value, keys) for key, value in data.items()}

    # If we got so far, the data must be a string (the encrypted value)
    try:
        data = decrypt_bytes(data.encode('utf-8'), keys)
